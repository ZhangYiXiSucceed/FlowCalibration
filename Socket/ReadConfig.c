...
int  ConfigGetKey(void *CFG_file, void *section, void *key, void *buf)
{
	...
	if((fp = fopen(TempBuff, "rb")) == NULL) return CFG_ERR_OPEN_FILE;
...
/*
	...
	ret = ConfigSetKey("Config.dat", "szy:net", "ip", "192.168.1.1");
	ret = ConfigSetKey("Config.dat", "szy:net", "port", "20000");
	...
	ret = ConfigGetKey("Config.dat", "szy:net", "port", buf);
	if (strcmp(buf,"")!=0) {
		port = atoi(buf);
	}
	...
	printf("\n buf=%s\n",buf);
	printf("\n port=%d\n",port);
	ret = ConfigGetKey("Config.dat", "szy:net", "ip", buf);
	printf("\n ip=%s\n",buf);

	ConfigSetKey("Config.dat","szy:net", "Email", "zhongyi.shen@moons.com.cn");

	ret = ConfigSetKey("Config.dat", "szy:net", "ip", "10.3.2.185");

	FileCopy("Config.dat", "Configbak.dat");

	printf("\nFile %s line%d\n",__FILE__,__LINE__);

	printf("\n******** This test is created by  szy********\n\n");
	*/
